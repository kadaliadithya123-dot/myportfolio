...
export const FloatingDock: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-3 left-1/2 -translate-x-1/2 z-40 flex max-w-[calc(100vw-1rem)] items-center">
      <motion.div
        initial={{ y: 50, opacity: 0 }}
        animate={{ y: 0, opacity: 1 }}
        transition={{ delay: 0.8, type: 'spring', stiffness: 300, damping: 25 }}
        className="glass-panel max-w-full overflow-x-auto px-1.5 py-1.5 sm:px-3 sm:py-2 rounded-2xl border border-white/15 flex items-center gap-1 sm:gap-1.5 shadow-2xl backdrop-blur-xl bg-black/60"
      >
          ...
          return (
            <div key={item.id} className="relative">
              ...
              <motion.button
                ...
                whileTap={{ scale: 0.9 }}
                className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-slate-300 hover:text-white hover:bg-white/10 transition-colors shrink-0"
        ...
        <div className="relative">
          ...
          <motion.button
            ...
            whileHover={{ scale: 1.25, y: -4 }}
            whileTap={{ scale: 0.9 }}
            className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-neon-cyan hover:bg-neon-cyan/15 transition-colors border border-neon-cyan/30 shrink-0"
